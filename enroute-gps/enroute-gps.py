#
#	Program Name:Python GPS Tracking (An Idea borrowed from a1ronzo)
#	Programmer Name:Eric Hamisi
...
def altitude():
	...
	file1 = open('temp.txt', 'w') #creates and opens a writable txt file
	file1.truncate() #erase contents of file
	shutil.copyfile('nmea.txt', 'temp.txt') #copy nmea.txt to temp.txt
	file1.close() #close writable file
	
	file1 = open('temp.txt', 'r') #open and read only
	try: #best to use try/finally so that the file opens and closes correctly
		for line in file1: #read each line in temp.txt
...
def position():
	...
	im = plt.imread('enroute-gps-Map.png')
